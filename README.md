...
mindmap
  root((Actions Stock Monitor))
    ...
    Storage & UI
      models.py
        dataclasses Product/DomainRun/RunSummary
      state.py
        JSON Load/Save
      dashboard.py
        HTML/Vue Dashboard Generation
    Network & Parsers
      http_client.py
        Connection Pools
        FlareSolverr integration
      parsers/
        registry.py Maps domains
        generic.py Default WHMCS
        greencloud.py Custom
        spa_store_api.py API Endpoints
    Notifications & Config
      telegram.py
        Async robust notifications
      Configuration Knobs
        Speed, Cloudflare, Telegram
      scripts/
        site_debug.py Single domain pipeline testing
```
